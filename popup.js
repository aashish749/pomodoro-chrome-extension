...
function renderAchievementHonors(streakData, canRecover, badgeData) {
  ...
  const { monthly = 0, lifetime = 0 } = badgeData || {};

  let html = "";

  // ===== STREAK HONOR CARD =====
  html += `
    <div class="honor-card honor-streak-container">
      <img
      ...
      <div class="honor-content">
        <h4 class="honor-title">Streak</h4>
        <div class="honor-details">
          <p style="margin: 4px 0;">
            Current: <span class="highlight">${currentStreak}</span> days
          </p>
          <p style="margin: 4px 0;">
            Longest: <span class="highlight">${longestStreak}</span> days
          </p>
          <p style="margin: 4px 0;">
            Streak Savers: <span class="highlight">${streakSavers}</span>
          </p>
          <p style="margin: 4px 0; font-size: 11px; color: #888;">
            Progress to next saver: <span class="highlight">${progressToNextSaver}</span>/14
          </p>
        </div>
      </div>
    </div>
  ...
  // ===== BADGE HONOR CARD =====
  html += `
    <div class="honor-card honor-badge-container">
      <img
        src="${chrome.runtime.getURL("badge (1).png")}"
        alt="Badge"
        class="honor-icon"
      />
      <div class="honor-content">
        <h4 class="honor-title">Badges</h4>
        <div class="honor-details">
          <p style="margin: 4px 0;">
            Lifetime: <span class="highlight">${lifetime}</span>
          </p>
          <p style="margin: 4px 0;">
            This Month: <span class="highlight">${monthly}</span>
          </p>
        </div>
      </div>
    </div>
